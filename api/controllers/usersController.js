...
module.exports.userDetails = async function(req, res) {
  ...
  if (token) {
    // verifies secret and checks if the token is expired
   let userDecodedVal =  await jwt.verify(token.replace(/^Bearer\s/, ''), config.authSecret, null);
  

   User.findOne({email: userDecodedVal.email}, function(err, user){
  ...
  // return res.json({ user: userDecodedVal })
  }
  else{
    return res.status(401).json({message: 'unauthorized'})
  }
